...
Текущая версия приложения в коде: `1.4.1`.
...
drawRandomExcludingRank(excludedRank: GoalkeeperCardRank, random: RandomGenerator): GoalkeeperCard | undefined;
...
После удара по воротам GK-карта обрабатывается отдельно от основной колоды:

- `GOALKEEPER_SAVE` - текущая GK-карта снимается с поля; из GK-колоды защищавшейся команды случайно выбирается новая GK-карта с rank, отличным от снятой карты. Выбранная карта ставится в `goalkeeper` slot, старая GK-карта возвращается в GK-колоду. Для UI логируется `GOALKEEPER_RANK_CHANGED` с `previousCard` и `nextCard`, а визуально rank прокручивается на той же карте.
- `GOAL_SCORED` - текущая GK-карта возвращается вниз GK-колоды защищавшейся команды, поле защищавшейся команды очищается, а новая GK-карта будет взята стандартным восстановлением поля в следующем ходе.
- `GOALPOST_HIT` - GK-карта не меняется, не возвращается в GK-колоду и новая GK-карта не тянется.
...
- текущая GK-карта снимается с поля;
- из `goalkeeperDeck` защищавшейся команды выбирается случайная GK-карта с другим rank;
- выбранная GK-карта ставится в `goalkeeper` slot;
- старая GK-карта возвращается в `goalkeeperDeck`;
- логируется `GOALKEEPER_RANK_CHANGED`;
- в UI rank прокручивается на той же визуальной GK-карте.
...
Если после успешного продвижения атаки нет карты в основной колоде и нет доступного подключенного полузащитника, логируется `ATTACK_DECK_EMPTY`, затем атака завершается и ход переходит сопернику. В UI это событие показывает flying message `No cards!`.
...
Для GK после сэйва дополнительно логируется:
