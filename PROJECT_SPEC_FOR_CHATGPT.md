...
Текущая версия приложения в коде: `1.3.5`.
...
- `GOALKEEPER_SAVE` - текущая GK-карта возвращается вниз GK-колоды защищавшейся команды, новая GK-карта берется сверху и остается в `goalkeeper` slot. Для UI логируется `GOALKEEPER_RANK_CHANGED` с `previousCard` и `nextCard`, а визуально rank прокручивается на той же карте.
